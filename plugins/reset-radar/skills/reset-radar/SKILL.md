...
Use this mode only when the user asks to show/open the floating radar. Never enable it implicitly during ordinary queries. On **Windows**, run `node scripts/reset-radar.mjs overlay start`: it opens an independent native card, does not modify/restart Codex or enable CDP, and requires the launching terminal to remain open. The Node bridge writes only safe display fields to the current user's local state directory; it never sends the saved Key to PowerShell. The first load refreshes immediately while still respecting a persisted server 429 deadline. The user may drag, collapse, close, or click refresh; duplicate clicks coalesce. On **macOS**, the card injects into the Codex main window through an unofficial CDP connection and follows the steps below. On Linux, state that regular queries and monitoring are supported but a floating card is unavailable.
