...
- Windows reads and writes validate the file owner and ACL using the system Windows PowerShell. New files are created with a private ACL; unsafe existing files or shared writable directories are rejected without changing their permissions or replacing your key. This also applies to custom key/state paths. If ACL verification is unavailable, access fails closed.
